...
// 按需加载（用于单个组件文件较大时）
// const Item = (resolve: any): any => require(["./Item.vue"], resolve);
// const dailyArticle = (resolve: any): any =>
//   require(["./DailyArticle.vue"], resolve);
...
export default class Index extends Vue {
  ...
  private showThemes: boolean = false;
  private themeId: number = 0;
  private list: Array<any> = [];
  private type: string = "recommend";
  private recommendList: Array<any> = [];
  private dailyTime: number = $.getTodayTime();
  private isLoading: boolean = false;
  private articledId: number = 0;
  ...
  private handleToTheme(id: number): void {
    // 改变菜单分类
    this.type = "daily";
    // 设置当前点击子类的主题日报 id
    this.themeId = id;
    // 清空中间栏的数据
    this.list = [];
    $.ajax.get("theme/" + id).then((res: any) => {
      // console.log(res);
      // 过滤掉类型为 1 的文章，该类型下的文章为空
      this.list = res.stories.filter((item: any): boolean => item.type !== 1);
    });
  }
  private handleToRecommend(): void {
    this.type = "recommend";
    this.recommendList = [];
